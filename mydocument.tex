...
\end{figure}
\end{framed}
I have used the \texttt{epsfig} package here - when compiling with latex (as opposed to pdflatex) only eps formatted images are acceptable, but \texttt{epsfig} is a little outdated. The other popular graphics package is \texttt{graphicx/graphics} which works with other image formats. 
...
\begin{figure}[H]
	\input{examplefig2}
	\epsfig{file=examplefig2.eps,scale=.550,clip=}
